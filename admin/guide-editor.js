(() => {
  ...
  function chemicalForm(c){
    ...
    edit.querySelector('#chemicalForm').addEventListener('submit',async e=>{
      ...
      if(hseStatus==='verified'&&(!/^https:\/\//i.test(sdsUrl)||!sdsLanguage||!hseDate||!hseHazards||!hsePpe||!hseFirst||!hseStorage)){alert('Для HMS=verified обязательны HTTPS SDS/HMS-ссылка, язык, дата проверки, опасности, СИЗ, первая помощь и хранение.');return;}
