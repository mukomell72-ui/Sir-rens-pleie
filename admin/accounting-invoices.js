(() => {
  ...
  async function paid(id){
    ...
    const m=String(method).trim().toLowerCase();if(!Object.keys(A.methods).includes(m)){alert('Допустимо: bank, card, cash, vipps, other');return;}
  ...
  async function credit(id){
    if(!confirm('Создать новый нумерованный кредит-нот и пометить исходный счёт как credited?'))return;
    try{
