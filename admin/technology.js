(() => {
  ...
  async function load(){
    ...
    photos=[];
    for(const photo of p){const {data}=await sb.storage.from('order-photos').createSignedUrl(photo.storage_path,1800);if(data?.signedUrl)photos.push({url:data.signedUrl});}
  }
  ...
  function render(){
    ...
    const orderItems=items.length?items.map(x=>`${esc(itemLabel(x.item_code))} × ${x.quantity}`).join(', '):(order.package_code==='full'?'Полный салон':esc(order.package_code||'—'));
  ...
  const serviceLabel=v=>({car:'салон автомобиля',sofa:'диван',chair:'кресло',mattress:'матрас'}[v]||v||'—');
