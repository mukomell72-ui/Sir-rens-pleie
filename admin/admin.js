...
async function dashboard(){
  ...
  main.innerHTML=`<div class="section-title"><div><h1>Главная</h1><p>Сегодняшняя работа и риски</p></div></div>${preview?'<div class="notice">Безопасный предпросмотр — реальная база не изменяется.</div>':''}<div class="grid"><div class="card metric"><span>Новые</span><strong>${count('new')}</strong></div><div class="card metric"><span>Ждут подтверждения</span><strong>${count('awaiting_confirmation')}</strong></div><div class="card metric"><span>В работе</span><strong>${count('in_progress')}</strong></div><div class="card metric"><span>Выручка сегодня</span><strong>${money(revenue)}</strong></div></div><div class="panel"><div class="panel-head"><span>Требуют внимания</span><span class="mini">STOP / HIGH RISK / новые</span></div>${attention.length?orderTable(attention):'<div class="empty">Срочных заказов нет.</div>'}</div>`;
  bindOrderRows();
}
function orderTable(rows){return `<div class="table-wrap"><table class="table"><thead><tr><th>Заказ</th><th>Клиент</th><th>Услуга</th><th>Статус</th><th>Цена</th><th>Риск</th></tr></thead><tbody>${rows.map(x=>`<tr class="order-row" data-id="${x.id}" tabindex="0"><td><b>${esc(x.order_no||'—')}</b></td><td>${esc(x.customer_name||'—')}<div class="mini">${esc(x.phone||'')}</div></td><td>${esc(x.service_type||'—')}</td><td>${statusLabel[x.status]||esc(x.status||'—')}</td><td>${x.final_price!=null?money(x.final_price):x.preliminary_price!=null?money(x.preliminary_price):'—'}</td><td><span class="risk ${(x.risk_level||'low').replace('_','-')}">${esc((x.risk_level||'LOW').toUpperCase())}</span></td></tr>`).join('')}</tbody></table></div>`;}
...
async function orders(){
  ...
  main.innerHTML=`<div class="section-title"><div><h1>Заказы</h1><p>Заявка → предложение → подтверждение → работа</p></div>${canManage()?'<button class="btn primary" id="manualOrder">+ Ручной заказ</button>':''}</div>${o.length?`<div class="panel">${orderTable(o)}</div>`:'<div class="card empty">Заказов пока нет.</div>'}`;
  bindOrderRows();
...
async function orderDetail(id){
  ...
  main.innerHTML=`<div class="section-title"><div><h1>${esc(o.order_no)}</h1><p>${esc(o.customer_name)} · ${esc(o.phone)}</p></div><button class="btn" id="backOrders">← Заказы</button></div><div class="detail-grid"><section class="card"><h3>Клиент и заказ</h3><div class="kv"><span>Услуга</span><b>${esc(o.service_type)}</b></div><div class="kv"><span>Адрес</span><b>${esc(o.address||'—')}</b></div><div class="kv"><span>Автомобиль</span><b>${esc([o.vehicle_plate,o.vehicle_brand,o.vehicle_model,o.vehicle_year].filter(Boolean).join(' ')||'—')}</b></div><div class="kv"><span>Загрязнение</span><b>${esc(o.contamination||'—')}</b></div><div class="kv"><span>Пятна / шерсть / запах</span><b>${o.stains?'пятна ':''}${o.pet_hair?'шерсть ':''}${o.odor?'запах':''||'—'}</b></div><div class="field"><label>Комментарий клиента</label><textarea readonly>${esc(o.customer_comment||'')}</textarea></div><div class="toolbar"><a class="btn" href="tel:${esc(o.phone)}">Позвонить</a><a class="btn" href="sms:${esc(o.phone)}">SMS</a></div></section><section class="card"><h3>Управление</h3><form id="orderForm"><div class="field"><label>Статус</label><select name="status" ${canManage()?'':'disabled'}>${allStatuses.map(s=>`<option value="${s}" ${o.status===s?'selected':''}>${statusLabel[s]}</option>`).join('')}</select></div><div class="field"><label>Риск</label><select name="risk" ${canManage()?'':'disabled'}>${['low','caution','high_risk','stop'].map(r=>`<option value="${r}" ${o.risk_level===r?'selected':''}>${r.toUpperCase()}</option>`).join('')}</select></div><div class="field"><label>Окончательная цена</label><input name="final_price" type="number" min="0" value="${o.final_price??''}" ${canManage()?'':'disabled'}></div><div class="field"><label>Исполнитель</label><select name="assigned_to" ${canManage()?'':'disabled'}><option value="">Не назначен</option>${staff.map(p=>`<option value="${p.id}" ${o.assigned_to===p.id?'selected':''}>${esc(p.display_name||p.role)}</option>`).join('')}</select></div><div class="field"><label>Внутренняя заметка</label><textarea name="note">${esc(o.internal_note||'')}</textarea></div>${canManage()?`<h4>Календарь</h4><div class="settings-grid"><div class="field"><label>Дата</label><input name="date" type="date" value="${localStart.date}"></div><div class="field"><label>Время</label><input name="time" type="time" value="${localStart.time}"></div><div class="field"><label>Длительность, мин.</label><input name="duration" type="number" min="30" step="15" value="${appt?Math.max(30,Math.round((new Date(appt.ends_at)-new Date(appt.starts_at))/60000)):o.estimated_minutes||240}"></div><div class="field"><label>Бронь</label><select name="tentative"><option value="true" ${appt?.tentative!==false?'selected':''}>Ожидает подтверждения</option><option value="false" ${appt?.tentative===false?'selected':''}>Подтверждена</option></select></div></div>`:''}<button class="btn primary" type="submit">Сохранить</button></form></section></div>${signed.length?`<section class="panel"><div class="panel-head">Фото клиента</div><div class="photo-grid">${signed.map(p=>`<a href="${p.url}" target="_blank"><img src="${p.url}" alt="Фото заказа"></a>`).join('')}</div></section>`:''}<section class="panel"><div class="panel-head"><span>Технологическая карта</span><a class="btn" href="technology.html?order=${encodeURIComponent(id)}" target="_blank">Открыть SIR Технолог</a></div><div class="cardless">${tech?`Риск: <b>${esc(tech.risk_level)}</b> · материал: ${esc(tech.material_guess||'не указан')}`:'Карта ещё не подтверждена. Для сложной поверхности сначала осмотр и тест.'}</div></section>`;
  main.querySelector('#backOrders').addEventListener('click',orders);
