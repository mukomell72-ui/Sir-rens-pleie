...
async function handleOffadmCommand(msg: any, text: string) {
  const m = String(text ?? "").match(/^\/offadm(?:@\w+)?\s+([^\s]+)\s+by\s+(.+)$/i);
  if (!m) return false;
...
if "await handleOffadmCommand(msg, text)" not in s:
    anchors = [
        '  if (await handleSetModer(msg, text)) return new Response("ok");',
        '  if (await handleStatsCommand(msg, text)) return new Response("ok");',
        '  const chatId = Number(msg.chat.id);',
    ]
    for a in anchors:
        if a in s:
            s = s.replace(a, '  if (await handleOffadmCommand(msg, text)) return new Response("ok");\n' + a, 1)
            break
    else:
        raise SystemExit("offadm route anchor not found")
