...
const VOICE_PRESETS: Record<string, VoicePreset> = {
  ...
  streamer: { voice: "Algenib", label: "злой стример", direction: "Очень эмоциональный, грубоватый мужской стримерский стиль: низкий хрипловатый тембр, резкая энергичная подача, короткие ударные фразы, раздражённая и дерзкая интонация, местами почти крик, но слова должны оставаться разборчивыми. Это самостоятельный синтетический голосовой стиль: не копируй и не выдавай голос за конкретного реального человека." },
};
...
function voicePresetKey(raw: string) {
  ...
  const aliases: Record<string,string> = {
    ...
    streamer:"streamer", стример:"streamer", злой:"streamer", aggressive:"streamer",
  };
  return aliases[k] ?? "";
}
...
function premiumVoicesHelp() {
  return `<b>🎙 Голосовые Premium</b>\n\n` +
    ...
    `/voice streamer текст — злой хрипловатый стримерский стиль\n\n` +
