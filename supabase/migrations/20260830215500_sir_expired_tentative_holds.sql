create or replace function private.prevent_appointment_overlap()
returns trigger
language plpgsql
security definer
...
begin
  if exists (
    ...
    where a.id is distinct from new.id
      ...
      and not (
        a.tentative=true
        and o.status in ('awaiting_confirmation','offer_sent')
        and o.confirmation_expires_at is not null
        and o.confirmation_expires_at < now()
      )
...
create or replace function public.calendar_free_windows(p_day date)
...
begin
  ...
  loop
    if v_row.starts_at>v_cursor then starts_at:=v_cursor; ends_at:=least(v_row.starts_at,v_day_end); return next; end if;
