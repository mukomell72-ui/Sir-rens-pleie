CREATE OR REPLACE FUNCTION public.generate_order_technology_card(p_order_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
...
begin
  ...
  if v_order.service_type='car' then
    ...
    v_surfaces:=array['textile'];
  elsif v_order.service_type='rug' then
    v_surfaces:=array['carpet'];
  end if;
  ...
  if v_order.service_type='rug'
     and coalesce(nullif(v_order.material_code,''),'unknown') <> 'synthetic'
     and v_risk<>'stop' then
    v_risk:='high_risk';
  end if;
  ...
  v_steps:=jsonb_build_array(
    ...
    'Конкретное средство, разведение и выдержка — только из manufacturer_verified карточки с HMS/SDS статусом verified или source_reviewed, без risk=stop. HIGH RISK и approval_required не назначаются автоматически.'
  );
  ...
  if v_order.service_type='rug' then
    v_steps:=v_steps||jsonb_build_array(
      'Ковёр: до влажной обработки подтвердить состав по маркировке/осмотру и выполнить тест стойкости цвета на незаметном участке.',
      'Для шерсти, вискозы, хлопка, смешанного/высокого ворса или неизвестного состава не применять автоматическое усиление: карта остаётся HIGH RISK до решения ответственного лица.'
    );
  end if;
  ...
  v_stop:=jsonb_build_array(
    ...
    'У выбранной химии отсутствует актуальный HMS/SDS-статус, либо карточка имеет risk=STOP.',
    'HIGH RISK или approval_required средство выбрано без явного подтверждения OWNER/ADMIN.'
  );
  if v_order.service_type='rug' then
    v_stop:=v_stop||jsonb_build_array(
      'Ковёр: STOP при переносе красителя, неизвестном составе без безопасной проверки, признаках усадки/деформации или отсутствии подтверждённой совместимой процедуры.'
    );
  end if;
  ...
  values(auth.uid(),'technology_draft_generated','order',p_order_id::text,jsonb_build_object('risk',v_risk,'surfaces',to_jsonb(v_surfaces),'hse_gate','v1'));
