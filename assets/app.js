(() => {
  ...
  const rules={};
  const T={
    ru:{back:'Назад',next:'Далее',send:'Отправить заявку',light:'Лёгкое',medium:'Среднее',heavy:'Сильное',special:'Особое состояние',prelim:'Ориентировочная цена',manual:'Оценка после осмотра',received:'Заявка получена',priceNotice:'Цена на сайте ориентировочная. Окончательную цену SIR подтверждает после проверки фотографий и осмотра состояния, материала, объёма и рисков — до начала работы. Дополнительные работы и повышение цены выполняются только после согласия клиента.',requestNotice:'Отправка заявки не заключает договор и не создаёт обязанности оплаты. Запись и окончательные условия действуют только после отдельного подтверждения SIR и клиента.'},
    no:{back:'Tilbake',next:'Neste',send:'Send forespørsel',light:'Lett',medium:'Middels',heavy:'Kraftig',special:'Spesiell tilstand',prelim:'Veiledende pris',manual:'Vurderes etter befaring',received:'Forespørselen er mottatt',priceNotice:'Prisen på nettstedet er veiledende. SIR bekrefter endelig pris etter kontroll av bilder og befaring av tilstand, materiale, omfang og risiko — før arbeidet starter. Tilleggsarbeid og prisøkning utføres bare etter kundens godkjenning.',requestNotice:'Innsending av forespørselen inngår ikke en avtale og medfører ingen betalingsplikt. Tidspunkt og endelige vilkår gjelder først etter separat bekreftelse fra SIR og kunden.'},
    en:{back:'Back',next:'Next',send:'Send request',light:'Light',medium:'Medium',heavy:'Heavy',special:'Special condition',prelim:'Indicative price',manual:'Assessed after inspection',received:'Request received',priceNotice:'The website price is indicative. SIR confirms the final price after reviewing photos and inspecting condition, material, scope and risk — before work begins. Additional work and price increases require the customer’s approval.',requestNotice:'Sending a request does not conclude a contract or create a payment obligation. The appointment and final terms apply only after separate confirmation by SIR and the customer.'}
  };
  ...
  function showDone(orderNo,photos={uploaded:0,failed:0},serverPrice=null){const root=currentRoot(),photoText=photos.uploaded?`Фото прикреплено: ${photos.uploaded}${photos.failed?`. Не загрузилось: ${photos.failed}`:''}.`:photos.failed?'Фото не прикрепились — их можно отправить менеджеру сообщением.':'';root.innerHTML=`<div class="wizard"><div class="step-title">${t('received')}</div><div class="summary"><div class="summary-row"><span>Номер</span><b>${esc(orderNo)}</b></div>${serverPrice!=null?`<div class="summary-row"><span>${t('prelim')}</span><b class="price">${fmt(serverPrice)}</b></div>`:''}<p>${t('priceNotice')}</p><p>${t('requestNotice')}</p>${photoText?`<p>${esc(photoText)}</p>`:''}</div><a class="btn primary" href="tel:${C.phonePrimary}">Позвонить SIR</a></div>`;}
