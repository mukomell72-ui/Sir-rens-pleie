(()=>{
  ...
  function touchlessPrewash(level){
    const selected=level==='light'?30:(level==='medium'?20:10);
    const mix=ratio(selected);
    return step('Бесконтактная предмойка','Green Star','Равномерно нанесите рабочий раствор на холодный кузов без механического трения. Дайте коротко поработать и тщательно смойте мойкой высокого давления до контактной мойки. Не давайте средству высохнуть.',{
      dilution:`${mix} Рабочая точка выбрана внутри официального диапазона Koch-Chemie для наружной мойки 1:5–1:30: лёгкая грязь 1:30, средняя 1:20, сильная 1:10.`,
      tool:'Распылитель/пенник для рабочего раствора + AVA P55 Go для полного смыва.',
      after:'После полного смыва переходите к контактной мойке. Не трите кузов до этого этапа.',
      warning:'Green Star — щелочной очиститель. Не наносить на горячий кузов, не давать высохнуть, перед применением проверить совместимость. Если защита кузова чувствительная, используйте более мягкую подтверждённую предмойку.',
  ...
  function makePlan(zone,level,dirt){
    ...
    if(zone==='body'){
      ...
      if(dirt==='iron'){
        steps.push(step('После мойки: металлические вкрапления','Reactive Rust Remover','На уже чистую и холодную поверхность нанесите средство, дождитесь визуальной реакции, не допускайте высыхания и тщательно смойте большим количеством воды.',{
          after:'После полного смыва при необходимости снова быстро пройти кузов шампунем и затем высушить.',
          risk:'CAUTION'
        }));
        risk='CAUTION';
      }else if(dirt==='tar'){
        steps.push(step('После мойки: битум / смола / клей','Eulex','На чистом холодном участке нанесите средство сначала на отдельную салфетку, локально обработайте загрязнение, коротко выждите и снимите остатки без сильного трения.',{
          after:'Обязательно вымыть обработанный участок шампунем и тщательно смыть перед сушкой.',
          risk:'HIGH RISK'
        }));
        risk='HIGH RISK';
      }else{
