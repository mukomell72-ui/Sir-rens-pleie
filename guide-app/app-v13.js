...
function card(x){const image=x.photo&&photos[x.photo]?photos[x.photo]:'';const thumb=image?`<div class="thumb"><img src="${h(image)}" alt="${h(x.n)}"></div>`:`<div class="thumb empty">Фото<br>не добавлено</div>`;const hero=image?`<div class="hero"><img src="${h(image)}" alt="${h(x.n)}"></div>`:`<div class="hero empty">Фото этой позиции пока не добавлено</div>`;const mark=x.mcolorname?`${x.m} · ${x.mcolorname}`:x.m,method=x.am?`<span class="pill">${h(x.am)}</span>`:'',url=safeUrl(x.buy?.url),canBuy=url!=='#',buyInline=canBuy?`<a class="buy-inline" href="${h(url)}" target="_blank" rel="noopener noreferrer" onclick="event.stopPropagation()">Купить ↗</a>`:'<span class="mini">Ссылка на магазин не добавлена</span>';return `<article class="card"><div class="head">${thumb}<div class="label" style="background:${h(x.col||'#35d2bd')};color:${h(x.tc||'#111')}">${h(x.m)}</div><div><div class="name">${h(x.n)}</div><div class="mini">${h(x.f)}</div><div class="meta"><span class="pill">${h(mark)}</span><span class="pill">${h(x.c)}</span></div>${method}${buyInline}</div><div class="arr">⌄</div></div><div class="body">${hero}<div class="sec"><b>Что чистить / назначение</b><p>${h(x.f)}</p></div>${x.t?`<div class="sec"><b>Способ нанесения / инструмент</b><p>${h(x.t)}</p></div>`:''}<div class="sec"><b>Разведение</b><p>${h(x.d)}</p></div><div class="sec"><b>Как чистить / применять</b><p>${h(x.u)}</p></div><div class="sec"><b>Что делать после</b><p>${h(x.a)}</p></div><div class="sec"><b>Риски и ограничения</b><p class="warn">${h(x.w)}</p></div><div class="sec"><b>Где купить</b><p>${canBuy?`<a class="buy-btn" href="${h(url)}" target="_blank" rel="noopener noreferrer">${h(x.buy?.shop||'Магазин')} ↗</a>`:'Ссылка не добавлена'}</p></div><div class="sec"><b>Наличие</b><p>${h(x.p)}</p></div><div class="tags">${(x.tags||[]).map(t=>`<span class="tag">${h(t)}</span>`).join('')}</div></div></article>`}
